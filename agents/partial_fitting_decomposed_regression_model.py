...
class PartialDecomposedRegressionModel:
    def __init__(self, regressor=DecisionTreeRegressor, memory_length=1, batch_size=100):
        self.models = None
    ...
    def init_episode(self, env, start_state):
        ...
        self.data_set_x = np.zeros((self.batch_size, (self._state_length + self._action_length) * self.memory_length))
        self.data_set_y = np.zeros((self.batch_size, self._state_length + 1))

        if self.models is None:
            self.models = []
            for i in range(self._state_length):
                self.models.append(SGDRegressor(warm_start=True))
            self.models.append(SGDRegressor(warm_start=True))

    def train(self):
        x_train, y_train = self._get_data_set()
    ...
    def partial_train(self, force=False):
        if self.batch_index == self.batch_size:

            for i in range(self.data_set_y.shape[1]):
                self.models[i].fit(self.data_set_x, self.data_set_y[:, i])
            self.is_trained = True
            self.batch_index = 0
        elif force:
            self.is_trained = True
    ...
    def add_observation(self, action, new_state, reward):
        ...
        if len(self.memory_states) >= self.memory_length:
            ...
            output_tuple = (*new_state, reward)
            self.data_set_x[self.batch_index, :] = input_tuple
            self.data_set_y[self.batch_index, :] = output_tuple
            self.batch_index += 1
...
if __name__ == "__main__":
    ...
    model = PartialDecomposedRegressionModel(memory_length=memory_length)
    ...
    for rep in trange(repetitions):
        ...
        model.init_episode(env, prev_state)
